...
def main(wanted_animal_ids = ["all"], wanted_session_ids=["all"], generate=False, skip_animal=[], skip_session=[]):
    ...
    #clean_animals(animals, skip_animal=skip_animal, skip_session=skip_session)

def clean_animals(animals, skip_animal=[], skip_session=[]):
    plotting = True
    bad_sessions = []
            ...
            try:
                ...
                for s2p_path in session.s2p_folder_paths:
                    ...
                    for part in session.session_parts:
                        if part in s2p_path:
                            if os.path.exists(os.path.join(s2p_path, "plane0", bin_fname)):
                ...
                #merged_unit = session.merge_units(generate=True, regenerate=False, delete_used_subsessions=False)
                ...
                bad_sessions.append([animal_id, session_id])
    print(f"Error cleaning sessions: {bad_sessions}")
...
if __name__ == "__main__":
    ...
    main()#skip_animal=["DON-009191"], skip_session=["20220225"]